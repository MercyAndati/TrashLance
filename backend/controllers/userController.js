  ...
  try {
    ...
    const [bookings, services, completedBookings, upcomingBookings, totalEarnings, monthlyEarnings] = await Promise.all(
      [
        ...
        req.user.role === "service_provider" ? Service.countDocuments({ provider: userId }) : Promise.resolve(0),
  ...
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(
      req.user._id,
      {
        ...
        "serviceProvider.basePrice": Number.parseFloat(pricing.basePrice) || 0,
    ...
    if (servicesOffered && servicesOffered.length > 0) {
      ...
      const servicePromises = servicesOffered.map((serviceType) => {
        ...
        return Service.create({
          ...
          pricing: {
            type: pricing.type,
            basePrice: Number.parseFloat(pricing.basePrice),
            unit: pricing.unit,
            ...
            additionalFees: pricing.additionalFee
              ? [
                  {
                    name: "Additional Fee",
                    amount: Number.parseFloat(pricing.additionalFee),
...
const getCollectorsByLocation = async (req, res) => {
  try {
    ...
    const collectors = await User.find({
      ...
      .select(
        "username avatar serviceProvider.companyName serviceProvider.rating serviceProvider.serviceLocations serviceProvider.serviceRadius serviceProvider.servicesOffered",
      )
      .populate("serviceProvider.servicesOffered", "name category")
      .sort({ "serviceProvider.rating.average": -1 })
    res.json({
