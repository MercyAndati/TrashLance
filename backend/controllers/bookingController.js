...
const createBooking = async (req, res) => {
  try {
    const { serviceProvider, service, scheduledDate, timeSlot, location, serviceDetails, pricing } = req.body
    ...
    if (
      provider.serviceProvider?.subscription?.endDate &&
      isSubscriptionExpired(provider.serviceProvider.subscription.endDate)
    ) {
    ...
    try {
      ...
      console.error("Notification/email/socket error during booking creation (non-critical):", notifyError)
...
const updateBookingStatus = async (req, res) => {
  try {
    ...
    // Send notifications and emit real-time updates in a separate try/catch
    try {
      const recipient =
        booking.customer.toString() === req.user._id.toString() ? booking.serviceProvider : booking.customer
    ...
    } catch (notifyEmitError) {
      console.error(
        "Error sending notification or emitting socket event after booking status update (non-critical):",
        notifyEmitError,
      )
      // Do NOT re-throw, as the main booking update was successful.
    }
...
const updatePaymentStatus = async (req, res) => {
  try {
    ...
    if (booking.serviceProvider.toString() !== req.user._id.toString() && req.user.role !== "admin") {
